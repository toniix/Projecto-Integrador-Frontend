...
export const ListUsers = () => {
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      ...
      errorToast("No se pudo eliminar el usuario.");
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      <div className="p-6 bg-gradient-to-r from-[#d9c6b0] to-[#f1eae7]">
        <h2 className="text-2xl font-bold text-[#3e0b05] mb-4">
            ...
            <tbody className="divide-y divide-[#d9c6b0]">
              {users.length > 0 ? (
                users.map((user) => (
                  <tr
                    key={user.id}
                    className="hover:bg-[#f1eae7] transition-colors"
                  >
                    <td className="p-3 text-[#1e1e1e]">{user.id}</td>
                    <td className="p-3 font-medium text-[#3e0b05]">
                      {user.firstName}
                    </td>
                    <td className="p-3 font-medium text-[#3e0b05]">
                      {user.email}
