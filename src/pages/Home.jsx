...
function Home() {
  ...
  const initialFilters = {
    ...
    categoryId: queryParams.get("categoryId")
      ? parseInt(queryParams.get("categoryId"))
      : null,
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.warn(
          "La respuesta de categorías no es un array, usando valores predeterminados"
        );
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          const data = await instrumentService.getProductsByCategory(
            filters.categoryId
          );
          ...
          console.error(
            "Error al buscar por categoría, usando búsqueda general:",
            err
          );
        ...
        <section className="flex flex-wrap justify-center gap-4 p-6 bg-[#F9F7F4] outline outline-1 outline-[#b08562]">
          ...
          {categories.map((category) => (
            <button
              ...
              style={{
                backgroundImage: `url(${
                  category.img || `/img/category-${category.id}.jpg`
                })`,
                backgroundSize: "cover",
                backgroundPosition: "center",
                filter:
          ...
          {products.length === 0 && hasActiveFilters ? (
            <div className="text-center py-10">
              ...
              <h3 className="text-xl font-semibold mb-2">
                No encontramos instrumentos que coincidan con tu búsqueda
              </h3>
                ...
                <div className="flex justify-between items-center">
                  <h2 className="text-2xl font-semibold text-[#3e0b05]">
                    {loading
                      ...
                      ? filters.categoryId && !filters.keyword
                        ...
                        : `Resultados ${
                            filters.keyword ? `para "${filters.keyword}"` : ""
                          }`
                  ...
                  {hasActiveFilters && !loading && pagination && (
                    <div className="text-sm text-[#757575]">
                      <span>
                        {pagination.totalElements} productos encontrados
                      </span>
                    </div>
                  )}
                </div>
